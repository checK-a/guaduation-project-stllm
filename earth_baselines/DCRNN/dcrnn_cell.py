...
from . import dcrnn_utils as utils
    ...
    def get_weights(self, shape, device):
        if shape not in self._params_dict:
            ...
            self._rnn_network.register_parameter('{}_weight_{}'.format(self._type, str(shape)),
                                                 nn_param)
        return self._params_dict[shape].to(device)

    def get_biases(self, length, bias_start=0.0, device=None):
        ...
        return self._biases_dict[length].to(device)
...
class DCGRUCell(torch.nn.Module):
    def __init__(self, num_units, adj_mx, max_diffusion_step, num_nodes, nonlinearity='tanh',
        ...
        # print(self._gconv_params.get_weights)
        # exit()
    ...
    def _fc(self, inputs, state, output_size, bias_start=0.0):
        ...
        weights = self._fc_params.get_weights((input_size, output_size), device=device)
        
        value = torch.sigmoid(torch.matmul(inputs_and_state, weights))
        biases = self._fc_params.get_biases(output_size, bias_start, device=device)
        value += biases
        return value
    ...
    def _gconv(self, inputs, state, output_size, bias_start=0.0):
        ...
        biases = self._gconv_params.get_biases(output_size, bias_start, device=inputs.device)
        x += biases
        # print(weights.shape,biases.shape,'==2=')
        # exit() # 5120 + 21
