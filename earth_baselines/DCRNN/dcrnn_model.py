...
class Seq2SeqAttrs:
    def __init__(self, args, data): 
        ...
        self.filter_type = getattr(args, "dcrnn_filter_type", "laplacian")
        self.num_nodes = data.m
        self.num_rnn_layers = args.n_layer
...
class DCRNNModel(nn.Module, Seq2SeqAttrs):
    def __init__(self, args, data):
        ...
        self._initialize_lazy_parameters(args)

    def _compute_sampling_threshold(self, batches_seen):
        return self.cl_decay_steps / (
                self.cl_decay_steps + np.exp(batches_seen / self.cl_decay_steps))

    def _initialize_lazy_parameters(self, args):
        # EARTH's DCGRU cell creates graph-convolution parameters on first use.
        # Run one tiny forward pass during construction so optimizers see all params.
        dummy = torch.zeros(1, args.window, self.num_nodes)
        with torch.no_grad():
            self.forward(dummy)
    ...
    def decoder(self, encoder_hidden_state, labels=None, batches_seen=None):
        ...
        go_symbol = torch.zeros(
            ...
            dtype=encoder_hidden_state.dtype,
        )
    ...
    def forward(self, inputs, labels=None, batches_seen=None):
        ...
        outputs = outputs.view(batch_size, self.decoder_model.horizon, self.num_nodes)
        return outputs, None
