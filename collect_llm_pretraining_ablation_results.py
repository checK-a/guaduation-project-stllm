...
def parse_log_name(path):
    ...
    seed_text, variant = rest.split("_", 1)
    return dataset, int(seed_text), variant


def parse_log(path):
    dataset, seed, variant = parse_log_name(path)
    rows = []
...
def main():
    parser = argparse.ArgumentParser(description="Collect LLM pretraining ablation results from suite logs.")
    parser.add_argument("--log_dir", type=str, default="llm_pretraining_ablation_logs")
    parser.add_argument("--out_csv", type=str, default="review/llm_pretraining_ablation_results.csv")
    parser.add_argument("--out_md", type=str, default="review/llm_pretraining_ablation_results.md")
    ...
    summary = (
        df.groupby(["dataset", "variant", "horizon"], as_index=False)[["mae", "rmse", "mape", "wmape"]]
        .agg(["mean", "std"])
        .reset_index()
    )
    summary.columns = [
    ...
    with out_md.open("w", encoding="utf-8") as f:
        f.write("# LLM Pretraining Ablation Results\n\n")
